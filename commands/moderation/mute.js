...
const wiodb = require('old-wio.db');
...
module.exports = {
    ...
    run: async (client, message, args) => {
        try {
            ...
            let muterole;
            let dbmute = await wiodb.fetch(`muterole_${message.guild.id}`);
            ...
            wiodb.set(`muteeid_${message.guild.id}_${mutee.id}`, userRoles)
            try {
                mutee.roles.set([muterole.id]).then(() => {
                    mutee.send(`_**Hello, You Have Been Muted In ${message.guild.name} for - ${reason || "No Reason"}**_`).catch(() => null)
                })
            } catch {
            ...
            if (reason) {
                ...
                const sembed2 = new MessageEmbed()
                    .setColor("GREEN")
                    .setDescription(`_**✅ ${mutee.user.username} was successfully muted**_`)
