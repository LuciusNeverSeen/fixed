...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        try {
            if (reason) {
                ...
                var sembed = new MessageEmbed()
                    .setColor("GREEN")
                    .setDescription(`_**✅${bannedMember.user.tag} has been unbanned for ${reason}**_`)
                message.channel.send(sembed)
            } else {
                message.guild.members.unban(bannedMember.user.id, reason)
                var sembed2 = new MessageEmbed()
                    .setColor("GREEN")
                    .setDescription(`_**✅${bannedMember.user.tag} has been unbanned**_`)
        ...
        let channel = (`921768885043208265`) //channel id
        if (!channel) return;
        ...
        var logChannel = message.guild.channels.cache.get(channel)
